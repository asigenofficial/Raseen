...
function barChart(monthly) {
  if (!monthly.length) return '<div class="empty">لا توجد بيانات بعد</div>';
  const max = Math.max(...monthly.map((m) => m.total), 1);
  return html`<div class="bar-chart">
    ${monthly.map((m) => raw(`<div class="bar" title="${esc(`${m.month}: ${money(m.total)}`)}">
        <i style="height:${Math.max(2, Math.round((m.total / max) * 130))}px"></i>
...
export async function render(view) {
  ...
  view.innerHTML = html`
    ...
    <div class="grid grid-4">
      ${raw(stat(icon.coins({ size: 22 }), 'blue', money(data.month.total), `مبيعات الشهر (${cur})`, `${num(data.month.count)} فاتورة`))}
      ${raw(stat(icon.calendar({ size: 22 }), 'green', money(data.today.total), `مبيعات اليوم (${cur})`, `${num(data.today.count)} فاتورة`))}
      ${raw(stat(icon.wallet({ size: 22 }), 'amber', money(data.all.remaining), `إجمالي المتبقي على العملاء (${cur})`, `${num(g('UNPAID').count + g('PARTIAL').count)} فاتورة غير مسددة`))}
      ${raw(stat(icon.calculator({ size: 22 }), 'red', money(data.all.tax), `ضريبة القيمة المضافة (${cur})`, 'إجمالي الضريبة المحصّلة'))}
    </div>
    ...
    <div class="grid grid-2 mt">
      ...
      <div class="card">
        ...
        <table class="tbl">
          <thead><tr><th>الحالة</th><th class="text-end">العدد</th><th class="text-end">الإجمالي (${cur})</th></tr></thead>
          <tbody>
            <tr><td>${raw(statusBadge('PAID', 'مسددة'))}</td><td class="text-end num">${num(g('PAID').count)}</td><td class="text-end num">${money(g('PAID').total)}</td></tr>
            <tr><td>${raw(statusBadge('PARTIAL', 'مسددة جزئياً'))}</td><td class="text-end num">${num(g('PARTIAL').count)}</td><td class="text-end num">${money(g('PARTIAL').total)}</td></tr>
            <tr><td>${raw(statusBadge('UNPAID', 'غير مسددة'))}</td><td class="text-end num">${num(g('UNPAID').count)}</td><td class="text-end num">${money(g('UNPAID').total)}</td></tr>
            <tr><td>${raw(statusBadge('CANCELLED', 'ملغاة'))}</td><td class="text-end num">${num(g('CANCELLED').count)}</td><td class="text-end num">${money(g('CANCELLED').total)}</td></tr>
          </tbody>
          <tfoot><tr><td>الإجمالي</td><td class="text-end num">${num(data.all.count)}</td><td class="text-end num">${money(data.all.total)}</td></tr></tfoot>
        </table>
      </div>
    </div>
    ...
    <div class="grid grid-2 mt">
      <div class="card pad0">
        ...
        <div class="table-wrap">
          <table class="tbl">
            <thead><tr><th>العميل</th><th class="text-end">فواتير</th><th class="text-end">الإجمالي</th><th class="text-end">المتبقي</th></tr></thead>
            <tbody>
              ${data.top_clients.length ? data.top_clients.map((c) => raw(`<tr>
                  ...
                  <td class="text-end num">${money(c.total)}</td>
                  <td class="text-end num" style="color:${c.remaining > 0 ? 'var(--danger)' : 'inherit'}">${money(c.remaining)}</td>
