  ...
  const draw = () => {
    ...
    : '')}
      ...
      <div class="grid grid-4">
        <div class="stat"><div style="min-width:0"><div class="stat-val num">${money(t.debit)}</div><div class="stat-lab">إجمالي المدين (فواتير)</div></div></div>
        <div class="stat"><div style="min-width:0"><div class="stat-val num">${money(t.credit)}</div><div class="stat-lab">إجمالي الدائن (مسدد)</div></div></div>
        <div class="stat"><div style="min-width:0"><div class="stat-val num" style="color:${balColor}">${money(t.closing_balance)}</div><div class="stat-lab">الرصيد الختامي (${esc(cur)})</div></div></div>
        <div class="stat"><div style="min-width:0"><div class="stat-val num">${money(t.open_invoices_remaining)}</div><div class="stat-lab">${num(t.open_invoices_count)} فاتورة غير مسددة</div></div></div>
      </div>
        ...
        <div class="table-wrap">
          <table class="tbl">
            <thead><tr><th style="width:30px">#</th><th>التاريخ</th><th>النوع</th><th>المستند</th>
              <th>الشركة</th><th>البيان</th><th class="text-end">مدين</th><th class="text-end">دائن</th><th class="text-end">الرصيد</th></tr></thead>
            <tbody>
              <tr class="row-open">
                <td colspan="6"><b>الرصيد الافتتاحي ${d.period.from ? `في ${dateAr(d.period.from)}` : ''}</b></td>
                <td colspan="2"></td><td class="text-end num"><b>${money(d.opening_balance_period)}</b></td>
              </tr>
    ...
    ? `<a href="#/invoice-view/${esc(e.doc_id)}">${esc(e.doc_number || '')}</a>` : esc(e.doc_number || '—')}</td>
                ...
                <td class="text-end num">${e.debit ? money(e.debit) : ''}</td>
                <td class="text-end num">${e.credit ? money(e.credit) : ''}</td>
                <td class="text-end num"><b>${money(e.balance_after)}</b></td>
              </tr>`).join(''))}
            ...
            <tfoot><tr>
              ...
              <td class="text-end num"><b>${money(t.debit)}</b></td>
              <td class="text-end num"><b>${money(t.credit)}</b></td>
              <td class="text-end num" style="color:${balColor}"><b>${money(t.closing_balance)}</b></td>
            </tr></tfoot>
          </table>
        </div>
      </div>`;
